...
`docker run --rm -v /local/path:/project andymardell/wpcs ./themes/my-theme`

Or to specify a standard, you can run

`docker run --rm -v /local/path:/project andymardell/wpcs --standard=WordPress-Core ./themes/my-theme`

## Relative paths

On OSX, at least, you can use `pwd` to mount your working directory as a volume:

`docker run --rm -v `pwd`/:/project andymardell/wpcs ./themes/my-theme`
